...
| `enable-agent-command-shims` | boolean | `true` | Controls whether Toastty prepends managed `codex` and `claude` wrappers into terminal `PATH` so manual invocations inside Toastty terminals report session status automatically. Set this to `false` if you do not want Toastty intercepting those commands. Agent menu launches still use their built-in instrumentation. New terminals and new shell processes pick up the change immediately after reload; existing shells may need a new shell or pane. |
