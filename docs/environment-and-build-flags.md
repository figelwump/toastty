...
| `TUIST_TOASTTY_TERMINAL_PROFILES_PATH` | `Project.swift` during `tuist generate` | unset | Non-secret local path copied to `TOASTTY_TERMINAL_PROFILES_PATH` in the generated `ToasttyApp-Release` Run scheme. Store it in `sv` and run `./scripts/dev/bootstrap-worktree.sh`; never use this forwarding mechanism for credentials because the resolved value is materialized in the gitignored scheme and inherited by app child processes. |
...
| Flag | Default | Effect |
|---|---|---|
| `TOASTTY_RUNTIME_HOME` | unset | Enables an isolated runtime sandbox for dev/test runs. When set, Toastty stores config, workspace layouts and annotation styles, generated agent-plugin state, the agent-hook starter template, Scratchpad documents, terminal profiles, pane restore journals, the default log path, `instance.json`, and UI-managed defaults under this directory instead of the shared user locations. The preferred automation socket path is also derived from this sandbox identity, but lives under the system temp directory to stay within Unix socket path-length limits. If that stable runtime-home socket path is already owned by a live Toastty listener, Toastty preserves the existing listener and falls back to a per-process sibling path such as `events-v1-<pid>.sock`. Explicit overrides such as `TOASTTY_USER_SKILLS_ROOT`, `TOASTTY_TERMINAL_PROFILES_PATH`, `TOASTTY_LOG_FILE`, and `TOASTTY_SOCKET_PATH` still win. |
| `TOASTTY_DEV_WORKTREE_ROOT` | unset | Enables a stable worktree-derived runtime sandbox for manual dev/debug/test runs when `TOASTTY_RUNTIME_HOME` is not set. Toastty derives `artifacts/dev-runs/worktree-<basename>-<hash>/runtime-home` under this worktree root, then applies the same isolation model described for `TOASTTY_RUNTIME_HOME`. Use this for repeated manual launches from one worktree. The Tuist-generated `ToasttyApp` and `ToasttyApp-Release` Run schemes already set it to `$(SRCROOT)`. |
| `TOASTTY_RUNTIME_LABEL` | unset | Optional run-owned label recorded in `instance.json` and automation ready files. With `TOASTTY_DEV_WORKTREE_ROOT` and no explicit `TOASTTY_RUNTIME_HOME`, it overrides the stable worktree label and derives `artifacts/dev-runs/worktree-<label>/runtime-home`. With explicit `TOASTTY_RUNTIME_HOME`, it is metadata only; the provided runtime-home path still wins. Repo automation helpers set this from their `RUN_ID` and verify it before driving socket actions. |
| `TOASTTY_USER_SKILLS_ROOT` | unset (`~/.toastty/skills` source) | Overrides the user skill-package source directory read by the app. Without the override, runtime-isolated instances intentionally read the real user's `~/.toastty/skills`; only generated snapshots, staging, and receipts move into the runtime home. Automated harnesses set this to a run-owned directory for full isolation. Toastty also advertises the resolved source path to supported managed agents through their reserved launch environment. |
...
| `TOASTTY_DEBUG_RESTORED_LAUNCH_CONTEXT` | unset | Debug-only diagnostic flag for restored terminal launches. When truthy, Toastty probes the restored shell command environment and logs whether `TOASTTY_PANEL_ID`, `TOASTTY_PANE_JOURNAL_FILE`, and `TOASTTY_LAUNCH_REASON` reached the shell. Off by default because the probe is diagnostic-only. |
...
| `TOASTTY_SIDEBAR_HOVER_DIAGNOSTICS` | unset | Temporary opt-in native/sidebar hover trace at info level. Read once at launch; accepts `1`, `true`, `yes`, or `on` (case-insensitive, whitespace trimmed). See [capture instructions](sidebar-hover-diagnostics.md). |
...
| `TOASTTY_RUNTIME_LABEL` | unset | Run-owned runtime label used by automation helpers to confirm the socket target belongs to the run before sending actions. |
...
| `ARCH` | remote machine arch | Used by the default macOS destination when custom options do not provide one. |
...
- Xcodebuild options after `--` supplement the platform defaults. The macOS defaults are `-workspace toastty.xcworkspace -scheme ToasttyApp -configuration Debug -destination "platform=macOS,arch=<remote-arch>"`; the iOS defaults are `-workspace ios/ToasttyMobile.xcworkspace -scheme ToasttyMobileApp -configuration Debug -parallel-testing-enabled NO`. An explicit workspace or project, scheme, configuration, parallel-testing setting, or destination wins.
- Without an explicit iOS destination, the wrapper resolves a compatible iPhone Simulator from `xcodebuild -showdestinations` and retains the complete probe output as `destination-probe.log` in the copied run artifacts.
