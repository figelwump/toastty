...
- A restored managed Codex or Claude panel with a valid native resume record runs
  the provider resume command instead of the profile `startupCommand`. If the
  record is missing or fails pre-launch validation, normal profile startup
  applies.
