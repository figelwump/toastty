...
- `~/.toastty/bin/` for ordinary runs, or `<runtime-home>/bin/` when runtime isolation is enabled and agent command shims are enabled
  - Managed `codex`, `cdx`, `claude`, `grok`, `opencode`, `mimo`, `mimocode`, and `pi` wrapper symlinks used to track manual agent invocations inside Toastty terminals.
...
- `$GROK_HOME/hooks/toastty-*.json` (default `GROK_HOME` is `~/.grok`) for managed Grok sessions
  - Ephemeral per-session Grok hook discovery files written at launch. Toastty removes the matching file when the managed session stops. On cold start, Toastty only deletes `toastty-*.json` files whose recorded owner process is no longer alive (or unowned legacy files older than a day), so a second Toastty instance sharing the same `GROK_HOME` does not wipe live hooks. Toastty does not install durable Grok hooks through a Get Started installer.
- Temporary launch artifact directories under the system temporary directory for managed agent sessions.
  ...
  - Grok launches include a temporary forwarder script plus `telemetry-failures.log` under the system temporary directory, alongside the session-scoped hook JSON under `$GROK_HOME/hooks/`. The hook JSON is removed when the managed session stops; temporary launch artifact directories are retained for a short grace period (about 10 minutes) so late hook invocations fail softly, then deleted automatically.
