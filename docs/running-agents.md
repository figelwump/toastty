...
4. **Starts a compatibility log watcher** that polls the session log file (every 250 ms) and maps supported Codex recording events to sidebar status updates. Current Codex CLI recordings drive start-of-turn **Working**, while legacy `codex_event` recordings continue to cover **Needs approval** and **Idle**
5. **Samples rendered terminal text** on Toastty's existing workspace-maintenance pulse and upgrades active Codex `Working` rows with best-effort tool progress such as `Running ...`, `Ran ...`, or `Executing shell commands`
6. **Uses the Codex notify hook** to map supported completion notifications to **Ready**. Current Codex documents `notify` as a turn-finished hook, not a tool-progress stream
