...
2. Toastty automatically exposes six session-only skills to supported managed Codex, Claude Code, Cursor, OpenCode, MiMo Code, and Pi launches; no skills setup is required. You can also add your own skills under `~/.toastty/skills` (see [User-created skills](#user-created-skills))
...
Use `worktree-create` to continue an agreed task in its own Git worktree and
background Toastty workspace. The parent preserves the plan and task identity;
...
These skills work with the repository being developed; `worktree-done` does not
...
Alongside the six shipped skills, Toastty delivers your own skills to managed
