...
- Commit the task changes as required by the repository, complete required checks for that commit, and show `ValidatedCommit: <full SHA>` with the branch, worktree path, checks, results, and any remaining limitations in the Scratchpad. For PR delivery, include the PR URL, repository, base branch and current base SHA, and head SHA; the PR head must equal `ValidatedCommit` and the task branch tip. Distinguish automated readiness, parent review, and human testing; never mark the latter two complete without their evidence. Any subsequent change to the local tip or live PR head, regardless of who made it, invalidates readiness until the tips agree and affected review and checks cover the new commit.
- The goal ends at readiness for parent review and human testing. Leave the workspace, Scratchpad, worktree, branch, and PR available. Do not merge or clean up as part of this goal. The user initiates `worktree-done` from the parent for assessment or integration. Parent review never authorizes a merge or cleanup; those still require explicit user approval. No permanent watcher or automatic parent wake-up is implied by this handoff.
- When resumed for accepted parent findings or user feedback, keep ownership of the fixes and update the same PR. Invalidate readiness before editing; record the new head and rerun affected review and verification before setting a new `ValidatedCommit`. Update human testing steps and identify which previous human checks need repeating. New behavior outside the agreed scope still needs the user.
