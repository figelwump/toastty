...
10. Open a new Toastty workspace for that worktree and launch the new terminal session with the bundled helper:
   ...
   - For the structured `agent.launch` path, the helper immediately scopes the launched child session to the newly created workspace and the parent workspace with `session scope set --session <child-session-id> --workspace <new-workspace-id> --workspace <parent-workspace-id>`. The parent workspace grant allows workspace-wide automation so the child can reply. The launch prompt records the parent panel and session IDs and authorizes a completion or blocked-status reply using `terminal.send-text` with `expectedSessionID`; a missing or replaced parent must be reported rather than retargeted. With `--no-scope-parent`, the helper retains child-only scope and does not establish a return route. This is a cooperative post-launch scope; treat a scope failure as a launch failure, but report that the workspace/session may already exist.
...
- Query each evidence panel by its explicit ID with `panel.browser.state`, following the bounded polling guidance in `toastty-capabilities`. This can start loading a background panel without selecting it. A successful creation response or a ready host does not prove navigation success; a detached host can still finish navigation.
- Navigation completion and an observed URL do not prove that an image displays correctly or a video plays. Use available background evidence and report what remains unverified; do not select a workspace/tab or move focus to strengthen the claim. If the running app lacks navigation status, report that limitation.
- If an artifact is renamed or regenerated, refresh or reopen it and verify the current version within those limits. In the final handback, identify the evidence panels opened and any artifacts that could not be displayed or whose display could not be verified.
...
- End the implementation goal at readiness for coordinator assessment, subject to the pending-check behavior above. Leave the workspace, optional Scratchpad, worktree, branch, and PR available. Do not merge or clean up as part of this goal. An assigned `project-orchestrator` can discover the handoff during its watch; without one, report it to the user for later `finisher`. The original parent need not remain active, and no coordinator is launched implicitly. Handoff or review does not authorize integration, publication, or cleanup.
...
- After a structured `agent.launch` succeeds, scope the child session to the created workspace and parent workspace by calling `session scope set --session <sessionID> --workspace <workspaceID> --workspace <parentWorkspaceID>` (child-only with `--no-scope-parent`) from the parent. Do not use `session scope set-current` for the child handoff; that command can only target the current parent session and panel. The helper performs this scope call automatically, verifies the returned explicit and effective workspace sets, and reports `scope_set`. Because the scope API runs after launch returns the child `sessionID`, this is cooperative workspace isolation, not a hard pre-exec sandbox.
