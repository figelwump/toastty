...
- Before handing back, open any screenshots, videos, or other visual artifacts captured for verification in the in-Toastty browser in the child's own workspace. Use the built-in `toastty-capabilities` skill to resolve that workspace and open the artifacts. Make remote captures accessible locally when needed, and confirm they display or play.
...
- the child requirements to open captured verification media and prepare locally testable changes in its own Toastty workspace before handing back, including relevant development commands and pages when known
