...
- **`cursor`** — Adds the same four Toastty skills and Cursor lifecycle hooks through launch-only `--plugin-dir` arguments; typed `cursor-agent` launches are treated as Cursor, while the generic `agent` alias is deliberately not shimmed
