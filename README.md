...
- **Workspace annotations** — agents, hooks, and CLI workflows can attach persistent keyed chips with optional links and consistent per-key colors across workspaces ([details](docs/cli-reference.md#action-run))
...
`Cmd+Ctrl+S` starts an unbound manual Scratchpad. Use the binding chip in the Scratchpad header to attach it to an active Toastty-managed agent session in the current tab. Supported managed Codex, Claude Code, Cursor, OpenCode, MiMo Code, and Pi sessions receive the `toastty-scratchpad` skill automatically, so you can usually ask the agent for a visual and let it create and bind its own Scratchpad on demand.
...
The TOML table name (the value in `[brackets]`) is the profile's internal ID. Toastty recognizes six well-known IDs that receive first-party instrumentation:

- **`codex`** — Automatically enables six Toastty skills only for managed sessions, and uses installed Codex status hooks when set up, with notify and session-recording fallback paths for compatibility. Typed `cdx` launches are treated as Codex too.
- **`claude`** — Automatically adds the same six Toastty skills for the managed process and injects lifecycle hooks that report session state back to the sidebar
- **`cursor`** — Adds the same six Toastty skills and Cursor lifecycle hooks through launch-only `--plugin-dir` arguments; typed `cursor-agent` launches are treated as Cursor, while the generic `agent` alias is deliberately not shimmed
- **`opencode`** — Automatically adds the same six Toastty skills and injects a temporary OpenCode-compatible status plugin through `OPENCODE_CONFIG_CONTENT`
- **`mimocode`** — Automatically adds the same six Toastty skills and injects a temporary OpenCode-compatible status plugin through `MIMOCODE_CONFIG_CONTENT`; typed `mimo` launches are treated as MiMo Code too
- **`pi`** — Automatically adds the same six Toastty skills and injects Toastty's bundled Pi extension for session, tool, and changed-file telemetry while preserving user Pi extensions
...
For Cursor, Toastty injects its plugin for a direct `cursor-agent` launch or when it can identify one unambiguous `cursor-agent` executable inside a supported wrapper; opaque wrappers still launch but receive only the base session context. For Pi, Toastty adds its own `--extension <toastty-pi-extension.js>` argument after the actual `pi` executable. User `--extension` arguments remain additive; `pi --no-extensions` or `pi -ne` disables Toastty's injected extension for that launch too. Any other profile ID launches the configured command with base `TOASTTY_*` session context but without agent-specific instrumentation. Custom agents can report status manually via the instance-specific CLI path exposed in `TOASTTY_CLI_PATH` — see the [full guide](docs/running-agents.md#custom-and-third-party-agents).

No skills setup is required. Supported managed Codex launches receive Toastty's six-skill plugin through a Toastty-owned plugin cache and profile overlay inside `CODEX_HOME` plus an injected `--profile toastty-managed` flag; the user's `config.toml` is never written and ordinary Codex sessions see no Toastty skills. Claude Code and Cursor receive immutable Toastty plugin copies through session-only `--plugin-dir` arguments, OpenCode and MiMo Code receive the same staged skills through launch-only config content, and Pi receives them through additive `--skill` flags. Cursor's plugin also carries its managed-session lifecycle hooks; Toastty does not rewrite `~/.cursor/hooks.json`. You can also add your own skills under `~/.toastty/skills/<name>/SKILL.md`; accepted packages are delivered to newly launched managed sessions alongside the built-in skills — see [User-created skills](docs/running-agents.md#user-created-skills). Run `"$TOASTTY_CLI_PATH" setup skills list` for a read-only inventory and exclusion diagnostics. After an app update, Toastty prepares the current bundled skills before restored supported sessions resume. Provisioning failures never block launch. Each supported agent shows a one-time nonblocking notice with `View Skills…`; the shared sheet lists built-in and user skills, explains the delivery models, and provides Codex status and repair controls.

Toastty never changes separately installed skills under `~/.codex/skills`, `~/.claude/skills`, `~/.cursor/skills`, or `~/.agents/skills`. If old unnamespaced Toastty skills appear alongside the `toastty:` plugin entries, remove those global copies manually.
...
The CLI can also drive higher-level workflows against a normal running Toastty instance. The built-in `worktree-create` skill resolves the current Toastty window, scopes the current parent session when needed, creates a background workspace, opens `WORKTREE_HANDOFF.md` as a local-document panel using Toastty's default markdown placement, and launches the new agent with structured `agent.launch` arguments for `cwd`, environment, `initialPrompt`, and optional setup commands. Fully custom startup commands remain available through the helper's explicit `--startup-command` path.

The child works toward human testing with its runtime's native persistent goal when available and permitted, and keeps progress, visuals, decisions, verification evidence, and testing steps in its Scratchpad. Once you have tried the result, ask the parent to use `worktree-done` to merge the named task and clean up. The parent discovers the validated commit, follows the target repository's integration rules, validates the landed result, and performs authorized session, workspace, worktree, and branch cleanup. See [Worktree tasks](docs/running-agents.md#worktree-tasks).

For development, repo-local skills can be linked into Claude and generic-agent skill directories with `scripts/agents/link-global-skills.sh`. Normal managed Codex, Claude Code, Cursor, OpenCode, MiMo Code, and Pi sessions use Toastty's automatic launch-scoped delivery instead. Run the script with `--help` to list its supported targets, selected skills, and cleanup options.
...
- `enable-agent-command-shims` in `~/.toastty/config` controls whether Toastty prepends managed wrappers into terminal `PATH` so manual built-in agent invocations (`codex`, `cdx`, `claude`, `cursor-agent`, `opencode`, `mimo`, `mimocode`, and `pi`) inside Toastty report session status automatically, including configured wrapper executables declared through `manualCommandNames`. The generic `agent` command is never shimmed. `manualCommandNames` is only for extra wrapper executable names; Codex integration checks apply to managed Codex launches from typed shims and UI launches. Set this flag to `false` if you do not want Toastty intercepting those commands. Agent menu launches still use their built-in instrumentation.
...
- Toastty persists one canonical workspace layout across display changes in `~/.toastty/workspace-layout-profiles.json`, or in the active runtime home's `workspace-layout-profiles.json` when runtime isolation is enabled. Legacy display-specific layouts are retained as recovery copies when an existing store is migrated.
