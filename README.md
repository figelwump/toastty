...
The child uses its runtime's native persistent goal when available and permitted, prepares a draft PR for PR-based implementation tasks, and keeps progress, visuals, decisions, verification evidence, and testing steps in its Scratchpad. The parent can use `worktree-done` for a review of design, codebase consistency, and interactions with relevant in-flight work before you test or authorize integration. Once you have tried the result, ask the parent to use the personal `worktree-done` skill to merge the named task and clean up. The parent discovers the validated commit, follows the target repository's integration rules, validates the landed result, and performs authorized session, workspace, worktree, and branch cleanup. See [Worktree tasks](docs/running-agents.md#worktree-tasks).
