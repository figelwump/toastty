...
- **Local documents and directory opens** — Open supported local files (`.md`, `.markdown`, `.mdown`, `.mkd`, `.yaml`, `.yml`, `.toml`, `.json`, `.jsonc`, `.jsonl`, `.ini`, `.conf`, `.cfg`, `.properties`, `.csv`, `.tsv`, `.xml`, `.sh`, `.bash`, `.zsh`) in tabs or splits as editable code views with line numbers. `Cmd`-clicking a local directory path in the terminal opens a split rooted at that directory.
