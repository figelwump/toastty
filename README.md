...
- **`cursor`** — Adds the same five Toastty skills and Cursor lifecycle hooks through launch-only `--plugin-dir` arguments; typed `cursor-agent` launches are treated as Cursor, while the generic `agent` alias is deliberately not shimmed
...
The CLI can also drive higher-level workflows against a normal running Toastty instance. The [personal `worktree-create` example](examples/skills/worktree-create/SKILL.md) resolves the current Toastty window, scopes the current parent session when needed, creates a background workspace, opens `WORKTREE_HANDOFF.md` as a local-document panel using Toastty's default markdown placement, and launches the new agent with structured `agent.launch` arguments for `cwd`, environment, `initialPrompt`, and optional setup commands. Fully custom startup commands remain available through the helper's explicit `--startup-command` path.

The child uses its runtime's native persistent goal when available and permitted, prepares a draft PR for PR-based implementation tasks, and records intent, decisions, verification, human testing steps, and deployment implications in the PR. Child Scratchpads are optional. The personal [`project-orchestrator` example](examples/skills/project-orchestrator/SKILL.md) watches PRs and assigned local tasks while you work, maintains a project Scratchpad dashboard, reviews interactions, and coordinates authorized local/remote integration and combined releases through each repository's workflows. It keeps merged but undeployed work visible. The coordinator or another finishing session can use `worktree-done` to assess, integrate, verify, and perform authorized cleanup without retaining the original parent. See [Worktree tasks](docs/running-agents.md#worktree-tasks).
...
To build or customize your own workflows, start with [the personal skill examples](examples/skills/README.md). Copy the packages you want into `~/.toastty/skills`; the examples are not automatically loaded from this repository or shipped in the built-in plugin.
