...
- **Live sidebar status** — working, needs-approval, ready, or error state for Codex, Claude, Grok, OpenCode, MiMo Code, and Pi sessions
...
- **`grok`** — Writes ephemeral per-session Grok hooks under `$GROK_HOME/hooks/` (default `~/.grok`) that report Working, Ready, Needs approval, and Error, plus native resume and subagent rows
