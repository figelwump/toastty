...
- **Local documents and directory opens** — Open supported local files in tabs or splits as editable code views with line numbers: Markdown, plain-text files (`.txt`), YAML/TOML/JSON/config files, CSV/TSV/XML, shell scripts, and common source files (`.swift`, `.js`, `.mjs`, `.cjs`, `.jsx`, `.ts`, `.mts`, `.cts`, `.tsx`, `.py`, `.go`, `.rs`). Backed local documents also expose `Open in Default App` as a header escape hatch. `Cmd`-clicking a local directory path in the terminal opens a split rooted at that directory.
