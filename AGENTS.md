...
- **`validate.sh`** — runs remote smoke validation on a macOS host over SSH, or a foreground-capable remote validation command when needed, then copies the remote artifacts back locally. Smoke runs fall back to local execution if remote preflight fails unless `--require-remote` is set.
