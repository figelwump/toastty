...
- If a validation flow would steal focus locally, prefer `TOASTTY_REMOTE_GUI_HOST=... ./scripts/remote/validate.sh ...` so remote smoke or foreground validation runs on the dedicated remote GUI machine instead of the current desktop.
...
- For remote validation, use `scripts/remote/validate.sh` with `TOASTTY_REMOTE_GUI_HOST`. It creates a disposable remote worktree, syncs the requested change scope into it, runs the requested smoke test or remote validation command there, and copies the artifacts back into `artifacts/remote-gui/<run-label>/`. By default, smoke runs fall back to a local run if remote preflight fails; pass `--require-remote` when the remote path itself must be validated.
