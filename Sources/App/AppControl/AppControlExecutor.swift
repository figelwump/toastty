    ...
    func resolveScratchpadExportTarget(
        ...
        if let sessionID = normalizedOptionalText(payload.stringValue("sessionID")) {
            ...
            if let rawWindowID = payload.stringValue("windowID") {
                ...
                guard target.windowID == windowID else {
                    throw AutomationSocketError.invalidPayload("sessionID Scratchpad does not belong to windowID")
                }
            }
            if let rawWorkspaceID = payload.stringValue("workspaceID") {
                guard let workspaceID = UUID(uuidString: rawWorkspaceID) else {
                    throw AutomationSocketError.invalidPayload("workspaceID must be a UUID")
                }
