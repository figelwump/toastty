...
struct ToasttyCLITests {
    ...
    @Test
    func sessionStatusRetriesTransientMissingActiveSession() throws {
        var attempts = 0
        let command = CLICommand.sessionStatus(
            sessionID: "sess-123",
            panelID: nil,
            kind: .working,
            summary: "exploring",
        ...
        let response = try ToasttyCLI.sendEventWithRetries(
            command: command,
            send: { envelope in
                attempts += 1
                if attempts == 1 {
                    return AutomationResponseEnvelope(
                        ...
                        error: AutomationResponseError(
                            ...
                            message: "sessionID does not identify an active session"
                        )
                    )
                }
                ...
                return AutomationResponseEnvelope(
                    ...
                    ok: true,
                    result: [
                        "eventType": .string("session.status"),
                    ],
                    error: nil
                )
            },
    ...
    @Test
    func sessionStatusStopsRetryingAfterBudgetExhausted() throws {
        var attempts = 0
        let command = CLICommand.sessionStatus(
            sessionID: "sess-123",
            panelID: nil,
        ...
        #expect(attempts == ToasttyCLI.transientMissingSessionRetryIntervals.count + 1)
        #expect(response.ok == false)
        #expect(response.error?.message == "sessionID does not identify an active session")
    }
}
