...
struct SessionRuntimeStoreTests {
    @Test
    func markSessionResumedLaunchShowsBadgeAndResumedIdleStatus() {
        ...
        store.markSessionResumedLaunch(sessionID: "sess-resumed", at: startedAt.addingTimeInterval(1))

        let status = store.sessionRegistry.workspaceStatuses(for: workspaceID).first
        #expect(status?.showsResumedBadge == true)
        #expect(status?.status == SessionStatus(kind: .idle, summary: "Waiting", detail: "Resumed"))
    }

    @Test
    func resumedBadgeClearsOnWorkingStatusButNotStartupReadyStatus() {
        let store = SessionRuntimeStore()
        let panelID = UUID()
        let workspaceID = UUID()
        let startedAt = Date(timeIntervalSince1970: 1_700_000_000)
        ...
        store.updateStatus(
            sessionID: "sess-resumed",
            status: SessionStatus(kind: .ready, summary: "Ready", detail: "Waiting for input"),
            at: startedAt.addingTimeInterval(2)
        )
        #expect(store.sessionRegistry.activeSession(sessionID: "sess-resumed")?.showsResumedBadge == true)

        store.updateStatus(
            sessionID: "sess-resumed",
            status: SessionStatus(kind: .working, summary: "Working", detail: "Editing files"),
            at: startedAt.addingTimeInterval(3)
        )
        #expect(store.sessionRegistry.activeSession(sessionID: "sess-resumed")?.showsResumedBadge == false)
    }

    @Test
    func resumedBadgeClearsOnFileUpdate() {
        let store = SessionRuntimeStore()
        let panelID = UUID()
        let startedAt = Date(timeIntervalSince1970: 1_700_000_000)
        ...
        store.updateFiles(
            sessionID: "sess-resumed",
            files: ["Sources/App/SidebarView.swift"],
            cwd: "/repo",
            repoRoot: "/repo",
            at: startedAt.addingTimeInterval(2)
        )
