...
final class BoundaryInteractionOverlayTests: XCTestCase {
    ...
    @MainActor
    func testCursorOwnershipCheckRestoresCursorWhileHoverRemainsInsideBoundary() throws {
        let (window, overlay) = makeWindowWithOverlay()
        defer { window.orderOut(nil) }
        let cursorProbe = installCursorProbe(on: overlay)
        ...
        cursorProbe.current = .iBeam
        overlay.performCursorOwnershipCheckForTesting()

        XCTAssertTrue(cursorProbe.current === NSCursor.resizeLeftRight)
        XCTAssertEqual(overlay.cursorOwnershipReassertionCount, 1)
    }

    @MainActor
    func testCursorOwnershipCheckStopsAfterPointerLeavesBoundary() throws {
        let (window, overlay) = makeWindowWithOverlay()
        defer { window.orderOut(nil) }
        let cursorProbe = installCursorProbe(on: overlay)
        ...
        XCTAssertTrue(cursorProbe.current === NSCursor.iBeam)
        XCTAssertEqual(overlay.cursorOwnershipReassertionCount, 0)
    }

    @MainActor
    func testScheduledCursorOwnershipLoopReassertsAndContinuesWhileHovered() throws {
        let (window, overlay) = makeWindowWithOverlay()
        defer {
            overlay.invalidate()
            window.orderOut(nil)
        ...
        XCTAssertTrue(overlay.hasScheduledCursorOwnershipCheck)
        runDeferredMainQueueWork()
        XCTAssertTrue(overlay.hasScheduledCursorOwnershipCheck)

        cursorProbe.current = .iBeam
        runScheduledCursorOwnershipWork()

        XCTAssertTrue(cursorProbe.current === NSCursor.resizeLeftRight)
        XCTAssertGreaterThanOrEqual(overlay.cursorOwnershipReassertionCount, 1)
        XCTAssertTrue(overlay.hasScheduledCursorOwnershipCheck)
        ...
        XCTAssertTrue(cursorProbe.current === NSCursor.resizeLeftRight)
        XCTAssertGreaterThanOrEqual(overlay.cursorOwnershipReassertionCount, 2)
    }

    @MainActor
    func testScheduledCursorOwnershipLoopStopsAfterDescriptorInvalidation() throws {
        let (window, overlay) = makeWindowWithOverlay()
        defer { window.orderOut(nil) }
        let cursorProbe = installCursorProbe(on: overlay)
        ...
        XCTAssertTrue(cursorProbe.current === NSCursor.iBeam)
        XCTAssertEqual(overlay.cursorOwnershipReassertionCount, 0)
        XCTAssertFalse(overlay.hasScheduledCursorOwnershipCheck)
    }
    ...
    @MainActor
    private func runScheduledCursorOwnershipWork() {
        RunLoop.main.run(until: Date(timeIntervalSinceNow: 0.03))
    }

    @MainActor
    private func installCursorProbe(
        on overlay: BoundaryInteractionOverlayView,
        initialCursor: NSCursor = .arrow
    ) -> CursorProbe {
        let probe = CursorProbe(current: initialCursor)
        overlay.currentCursorProvider = {
            probe.current
        }
        overlay.cursorSetter = { cursor in
            probe.current = cursor
        }
        return probe
    }
}
...
private final class CursorProbe {
    var current: NSCursor

    init(current: NSCursor) {
