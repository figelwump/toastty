...
final class ToasttyConfigStoreTests: XCTestCase {
    ...
    func testEnsureTemplateExistsWritesCommentedExamples() throws {
        ...
        XCTAssertEqual(
            ...
            # managed agent wrappers into terminal PATH so supported manual
            # invocations report session status automatically. Cursor uses
            # cursor-agent; Toastty never intercepts the generic agent command.
