...
final class CommandPaletteCatalogTests: XCTestCase {
    func testBuiltInCommandMetadataUsesStableIdentifiers() {
        ...
        XCTAssertEqual(ToasttyBuiltInCommand.sendDiagnostics.id, "diagnostics.snippet.copy")
    ...
    func testSharedSkillsCommandKeepsStableIDAndNamesBothAgentSearchPaths() {
        let command = ToasttyBuiltInCommand.manageToasttySkills

        XCTAssertEqual(command.id, "agent.toastty-skills.manage")
    ...
    func testStaticCatalogExposesExpectedBuiltInsInStableOrder() {
        ...
        XCTAssertEqual(
            ...
            [
                ...
                ToasttyBuiltInCommand.sendDiagnostics.id,
    ...
    func testCatalogFiltersUnavailableCommands() {
        ...
        actions.canSendDiagnosticsValue = false
        ...
        XCTAssertFalse(commands.contains(where: { $0.id == ToasttyBuiltInCommand.sendDiagnostics.id }))
    ...
    func testCatalogInvocationsExecuteAgainstOriginWindowID() throws {
        ...
        XCTAssertTrue(actions.execute(try XCTUnwrap(commands.first(where: { $0.id == ToasttyBuiltInCommand.sendDiagnostics.id })).invocation, originWindowID: originWindowID))
        ...
        XCTAssertEqual(actions.sentDiagnosticsWindowIDs, [originWindowID])
        XCTAssertEqual(
