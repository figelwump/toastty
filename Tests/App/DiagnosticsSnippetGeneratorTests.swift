...
final class DiagnosticsSnippetGeneratorTests: XCTestCase {
    func testSnippetShellQuotesBakedCLIPath() {
        ...
        XCTAssertTrue(snippet.contains("include it with --contact"))
        XCTAssertTrue(snippet.contains("do not edit the diagnostics JSON by hand and do not re-run collection just to add contact"))
        XCTAssertTrue(snippet.contains("Shell-quote contact text as one argument"))
        XCTAssertTrue(snippet.contains("Run exactly one command"))
        XCTAssertTrue(snippet.contains("With contact: \"<TOASTTY_CLI_RESOLVED>\" diagnostics submit --file \"<TOASTTY_DIAGNOSTICS_JSON>\" --yes --contact \"<NAME/EMAIL PROVIDED BY USER>\""))
        XCTAssertTrue(snippet.contains("Without contact: \"<TOASTTY_CLI_RESOLVED>\" diagnostics submit --file \"<TOASTTY_DIAGNOSTICS_JSON>\" --yes"))
