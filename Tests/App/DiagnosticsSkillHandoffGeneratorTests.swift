...
final class DiagnosticsSkillHandoffGeneratorTests: XCTestCase {
    func testPromptPointsAtBundledSkillAndCLIWithoutDuplicatingWorkflow() {
        ...
        XCTAssertTrue(prompt.contains("Do not rely on skill discovery or TOASTTY_SKILLS_ROOT."))
        XCTAssertTrue(prompt.contains("literal filesystem paths with no quotes included"))
        XCTAssertTrue(prompt.contains("wait for my explicit approval before uploading anything"))
        XCTAssertTrue(prompt.contains("never infer or discover contact information"))
        XCTAssertTrue(prompt.contains("stop and show me the exact error instead of guessing"))
