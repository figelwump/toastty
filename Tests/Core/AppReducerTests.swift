...
struct AppReducerTests {
    ...
    @Test
    func updateTerminalPanelMetadataClearsRestoredSemanticTitleOnSemanticTitleUpdate() throws {
        var state = AppState.bootstrap()
        let reducer = AppReducer()
        let workspaceID = try #require(state.windows.first?.selectedWorkspaceID)
        let panelID = try #require(state.workspacesByID[workspaceID]?.focusedPanelID)

        // Seed state with a restored semantic title (simulating post-restore).
        guard case .terminal(var terminalState) = state.workspacesByID[workspaceID]?.panels[panelID] else {
            Issue.record("expected terminal panel")
            return
        }
        terminalState.restoredSemanticTitle = "bundle exec rspec"
        terminalState.profileBinding = TerminalProfileBinding(profileID: "zmx")
        state.workspacesByID[workspaceID]?.panels[panelID] = .terminal(terminalState)

        // A semantic (non-path) runtime title clears the restored title.
        #expect(
            reducer.send(
                .updateTerminalPanelMetadata(panelID: panelID, title: "vim", cwd: nil),
                state: &state
            )
        )

        guard case .terminal(let updatedState) = state.workspacesByID[workspaceID]?.panels[panelID] else {
            Issue.record("expected terminal panel")
            return
        }
        #expect(updatedState.title == "vim")
        #expect(updatedState.restoredSemanticTitle == nil)
        try StateValidator.validate(state)
    }

    @Test
    func updateTerminalPanelMetadataPreservesRestoredSemanticTitleOnPathTitleUpdate() throws {
        var state = AppState.bootstrap()
        let reducer = AppReducer()
        let workspaceID = try #require(state.windows.first?.selectedWorkspaceID)
        let panelID = try #require(state.workspacesByID[workspaceID]?.focusedPanelID)
        ...
        // A path-like title should NOT clear the restored semantic title.
        #expect(
            reducer.send(
                .updateTerminalPanelMetadata(panelID: panelID, title: "/tmp/project", cwd: nil),
                state: &state
            )
        )
        ...
        #expect(updatedState.title == "/tmp/project")
        #expect(updatedState.restoredSemanticTitle == "bundle exec rspec")
        try StateValidator.validate(state)
    }
