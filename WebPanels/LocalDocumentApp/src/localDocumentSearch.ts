...
function applyPreviewSearch(
  ...
  if (query.length === 0 || root === null) {
    return emptySearchState(query);
  }

  const segments = textSegmentsFor(root);
...
export function useLocalDocumentSearchController(options: {
  ...
  const { content, isEditing, previewRootRef, textareaRef } = options;
  ...
  const applyCommand = React.useCallback((command: LocalDocumentPanelSearchCommand) => {
    ...
    return nextState;
  }, [isEditing, previewRootRef, textareaRef]);
  ...
  }, [applyCommand, isEditing, previewRootRef]);
}
