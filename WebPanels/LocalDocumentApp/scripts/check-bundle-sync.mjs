...
const checkedInOutputDir = resolve(
  packageRoot,
  "../../Sources/App/Resources/WebPanels/local-document-panel"
);
const generatedOutputDir = mkdtempSync(
  join(tmpdir(), "toastty-local-document-panel-")
);
const expectedFiles = [
  "index.html",
  "local-document-panel.css",
  "local-document-panel.js",
  "onig.wasm"
];
...
try {
  const build = spawnSync(process.execPath, [resolve(packageRoot, "scripts/build.mjs")], {
    ...
    env: {
      ...process.env,
      TOASTTY_LOCAL_DOCUMENT_PANEL_OUTPUT_DIR: generatedOutputDir
    },
    encoding: "utf8"
  });
  ...
  if (mismatchedFiles.length > 0) {
    process.stderr.write(
      [
        "Checked-in local document panel assets are out of sync.",
        "Run `npm run build` in WebPanels/LocalDocumentApp and commit the updated bundle.",
