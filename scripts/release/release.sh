...
resign_exported_app_for_distribution() {
  ...
  # Gatekeeper only accepts the final bundle if nested helpers plus bundled
  # launch helpers are re-signed inside-out before the framework and app are
  # sealed. Keep the Sparkle list aligned with the vendored framework layout.
  ...
  codesign_path_preserving_entitlements "$bundled_agent_shim_path"
